...
def main():
    ...
    #3lrs_GO = {'cellcycle':1e-4, 'derisi':1e-4, 'eisen':1e-4, 'expr':1e-4, 'gasch1':1e-4, 'gasch2':1e-4, 'seq':1e-4, 'spo':1e-4}
    ...
    #     train_to_eval, test_to_eval = torch.tensor(train_to_eval, dtype=torch.uint8),  torch.tensor(test_to_eval, dtype=torch.uint8)
    # else:
    #     train, val, test = initialize_dataset(dataset_name, datasets)
    #     train_to_eval, val.to_eval, test_to_eval = torch.tensor(train_to_eval, dtype=torch.uint8), torch.tensor(val.to_eval, dtype=torch.uint8), torch.tensor(test_to_eval, dtype=torch.uint8)
    ...
    DATA_PATH='/mnt/2ndHDD/nfs/ship/skin/Data/ipad/'
    ...
    data= dataset.SkinFolder(root=DATA_PATH, transform=transformations)
    dset_train,dset_test=torch.utils.data.random_split(data, [560, 133])
    ...
    # Create the model
    model = ConstrainedFFNNModel(input_dims, hidden_dim, output_dims, hyperparams, R)
    model.to(device)
    print("Model on gpu", next(model.parameters()).is_cuda)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    criterion = nn.BCELoss()
    train_to_eval=torch.ones(12,dtype=torch.uint8)
    test_to_eval=torch.ones(12,dtype=torch.uint8)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (x, labels) in enumerate(train_loader):
            ...
            loss = criterion(train_output[:,train_to_eval], labels[:,train_to_eval]) 
    ...
    score = average_precision_score(y_test[:,test_to_eval], constr_test.data[:,test_to_eval], average='micro')
